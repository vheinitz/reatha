...
/**
	 * Perform Tank Auth Authentication
	 *
	 * @param string $username The username to validate
	 * @param string $password The password to validate
	 * @return boolean
	 */
	protected function _perform_tank_auth($username = '', $password = NULL)
	{
		if (empty($username))
		{
		...
		if ($hasher->CheckPassword($password, $user->password)) {
			...
			log_message('info','rest_controller/tank_auth | auth error');
			return FALSE;
		}		
	...
	protected function _check_login($username = '', $password = NULL)
	{
		...
		if ($auth_source == 'tank_auth')
		{
			log_message('debug', 'performing Tank Auth authentication for $username');
			return $this->_perform_tank_auth($username, $password);
		}		
